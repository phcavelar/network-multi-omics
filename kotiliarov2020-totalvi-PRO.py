...
adata = sc.AnnData(np.zeros([data["rna"].X.shape[0],1]), obs = data["rna"].obs, var=data["rna"].var[:1])
#adata.X = data["rna"].layers['count'].toarray()
adata.obsm['proteins'] = data["cite"].layers['count'].toarray()
...
for repetition in range(NUM_REPS):
    ...
    z.write(os.path.join(DATA_PATH,RESULTS_PATH,f"kotliarov2020-totalviPRO-{repetition}.h5ad"))
    os.makedirs(os.path.join(DATA_PATH,MODEL_DIR), exist_ok=True)
    vae.save(os.path.join(DATA_PATH,MODEL_DIR,f"kotliarov2020-totalviPRO-{repetition}.model"))
    HISTORY_KEYS = [
        'train_loss_step',
        'train_loss_epoch', 'validation_loss',
        'elbo_train', 'elbo_validation',
    ...
    history_df.to_csv(os.path.join(DATA_PATH,f"kotliarov2020-totalviPRO-{repetition}-history.csv.gz"))

# %%
# Load first model for diagnostic plots
z = sc.read_h5ad(os.path.join(DATA_PATH,"kotliarov2020-totalviPRO-0.h5ad"))
history_df = pd.read_csv(os.path.join(DATA_PATH,"kotliarov2020-totalviPRO-0-history.csv.gz"), index_col="epoch")
