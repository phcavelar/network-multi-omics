...
with tqdm(total=len(TCGA_cancers)*NUM_REPS) as counter:
    ...
    for cancer in TCGA_cancers:
        ...
        for repetition in range(NUM_REPS):
            if os.path.exists(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-{repetition}.h5ad")):
                counter.update()
                continue
...
for cancer in TCGA_cancers:
    z = sc.read_h5ad(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-0.h5ad"))
    history_df = pd.read_csv(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-0-history.csv.gz"))
    subtype_cols = subtype_cols_per_cancer[cancer]
    stage_col = stage_col_per_cancer[cancer]
    clin_cols = ['gender', *subtype_cols, stage_col]
...
for cancer in TCGA_cancers:
    ...
    index_in_all = sorted(index_in_all)
    clin = clin.loc[index_in_all]
    agecol = [c for c in clin.columns if 'age_at_initial_pathologic_diagnosis' in c.lower()][-1]
    print(
        cancer,
        clin.shape,
        (clin[clin_cols].gender.value_counts()/clin.shape[0]*100)[[g for g in ["MALE","FEMALE"] if g in clin.gender.unique()]].round(2).to_dict(),
        f"${clin[agecol].mean():.2f}\\pm{clin[agecol].std():.2f}$",
    )
    if clinall is None:
        clinall = clin[["gender",agecol]].rename(columns={agecol:"age"})
    else:
        newclin = clin[["gender",agecol]].rename(columns={agecol:"age"})
        clinall = pd.concat([clinall, newclin])

print(
    "all",
    clinall.shape,
    (clinall.gender.value_counts()/clinall.shape[0]*100)[[g for g in ["MALE","FEMALE"] if g in clinall.gender.unique()]].round(2).to_dict(),
    f"${clinall['age'].mean():.2f}\\pm{clinall['age'].std():.2f}$",
)
