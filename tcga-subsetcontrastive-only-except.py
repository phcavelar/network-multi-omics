...
with tqdm(total=len(TCGA_cancers)*NUM_REPS*len(layers)*2) as counter:
    for cancer in TCGA_cancers:
        ...
        for data_type in data:
            # Only this data type
            only_this_data = {data_type: data[data_type]}
            for repetition in range(NUM_REPS):
                if os.path.exists(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-only-{data_type}-{repetition}.h5ad")):
                    counter.update()
                    continue
                gae = SubsetContrastive(
                    only_this_data,
                    {k:nn.MSELoss() for k in only_this_data},
                    200,#0.2,
                    num_epochs=256,
                )
                tr_log = gae.train(only_this_data)
                Xs = [torch.tensor(only_this_data[m].X, dtype=torch.float) for m in gae.modalities]
                As = [torch.tensor(np.stack(only_this_data[m].obsp["connectivities"].nonzero()), dtype=torch.long) for m in gae.modalities]
                ...
                adz.write(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-only-{data_type}-{repetition}.h5ad"))
                os.makedirs(os.path.join(DATA_PATH,MODEL_DIR), exist_ok=True)
                #gae.save(os.path.join(DATA_PATH,MODEL_DIR,f"tcga-{cancer}-subsetcontrastive-only-{data_type}-{repetition}.model"))
                history_df = pd.DataFrame(tr_log)
                history_df.to_csv(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-only-{data_type}-{repetition}-history.csv.gz"))
                counter.update()
            # All data types except this one
            except_this_data = {d: data[d] for d in data if d!=data_type}
            for repetition in range(NUM_REPS):
                counter.write(f"except-{data_type}-{repetition}")
                if os.path.exists(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-except-{data_type}-{repetition}.h5ad")):
                    counter.update()
                    continue
                gae = SubsetContrastive(
                    except_this_data,
                    {k:nn.MSELoss() for k in except_this_data},
                    200,#0.2,
                    num_epochs=256,
                )
                tr_log = gae.train(except_this_data)
                Xs = [torch.tensor(except_this_data[m].X, dtype=torch.float) for m in gae.modalities]
                As = [torch.tensor(np.stack(except_this_data[m].obsp["connectivities"].nonzero()), dtype=torch.long) for m in gae.modalities]
                _, __, z = gae.model(Xs, As)
                ...
                os.makedirs(os.path.join(DATA_PATH,RESULTS_PATH), exist_ok=True)
                adz.write(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-except-{data_type}-{repetition}.h5ad"))
                os.makedirs(os.path.join(DATA_PATH,MODEL_DIR), exist_ok=True)
                #gae.save(os.path.join(DATA_PATH,MODEL_DIR,f"tcga-{cancer}-subsetcontrastive-except-{data_type}-{repetition}.model"))
                history_df = pd.DataFrame(tr_log)
                history_df.to_csv(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-except-{data_type}-{repetition}-history.csv.gz"))
                counter.update()
...
for cancer in TCGA_cancers:
    z = sc.read_h5ad(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-only-rna-0.h5ad"))
    history_df = pd.read_csv(os.path.join(DATA_PATH,RESULTS_PATH,f"tcga-{cancer}-subsetcontrastive-only-rna-0-history.csv.gz"))
    subtype_cols = subtype_cols_per_cancer[cancer]
    stage_col = stage_col_per_cancer[cancer]
    clin_cols = ['gender', *subtype_cols, stage_col]
